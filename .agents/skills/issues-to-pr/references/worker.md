...
5. 検証記録をIssue別の既存記録場所へ残す。指定がなければ `packages/marionette_agent/docs/verification/issue-<number>.md` を使う。Issueが明示した記録場所を優先し、過去文書に存在しないファイルへの参照だけなら現状を確認して整備する。実行コマンド・期待/実際の結果・検証対象commit・環境・画像/動画の対応・人間の再現手順を記録する。URIや機密情報を残さない。
