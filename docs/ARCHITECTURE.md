...
RecordingManagerは開始前にdeviceを予約し、backendの開始確認後に返す。start時の終了競合でも遅れて生成されたhandleを停止する。stopの要求期限超過後も終了処理を保持し、終わるまで同じdeviceへ別録画を開始しない。Androidの自動終了も同じfinalizationへ合流する。closeとdaemonのSIGINT/SIGTERMでは録画を確定してから所有権を解放する。
