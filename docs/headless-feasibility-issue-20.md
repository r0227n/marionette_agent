...
> 更新: 以下は当初の調査時点の記録です。その後、利用者から実装と「各プラットフォームの実行環境を使う」ヘッドレス録画の指示を受けました。今回の実装・検証は[Issue #20検証](../packages/marionette_agent/docs/verification/issue-20.md)と[SPEC](SPEC.md#flutterアプリのヘッドレス録画issue-20)を正とします。その後の追加依頼により、testerと各実行環境を明示選択するlaunchを実装しています。[最新の利用ガイド](ja/headless.ja.md)を参照してください。
